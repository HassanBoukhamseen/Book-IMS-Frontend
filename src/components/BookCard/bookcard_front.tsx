...
function process_authors(author: string): string {
  const limit = 25;
  author = author.split(' ').map(name => name.charAt(0).toUpperCase() + name.slice(1)).join(' ');
  return author.length >= limit ? author.slice(0, limit - 2) + '...' : author;
}
...
function process_genre(genre: string): string {
  const limit = 20;
  genre = genre.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
  return genre.length >= limit ? genre.slice(0, limit - 2) + '...' : genre;
}

const BookCardFront: React.FC<BookCardFrontProps> = ({ book, loggedIn, token, onImageClick }) => {
  const [liked, setLiked] = useState<boolean>(loggedIn && book.liked ? true : false);
  ...
  const toggleLike = async () => {
    if (!loggedIn) {
      ...
      if (book.liked === 0) {
        await registerLikedBook(book.book_id, token);
        book.liked = 1;
      } else {
        await removeLikedBook(book.book_id, token);
        book.liked = 0;
      }
      setLiked(!liked);
    }
        ...
        <div className={styles["book-card-front"]}>
            <div className={styles["card-header"]}>
                <div className={styles["book-info"]}>
                    <p className={styles["book-title"]}>{process_titles(book.title)}</p>
                    <div className={styles["author-year"]}>
                        <p style={{ "background": "transparent", "fontSize": "12px", "marginLeft": "6%" }}>{process_authors(book.author_name)}</p>
                        <p style={{ "background": "transparent", "fontSize": "12px", "marginLeft": "10%" }}>{book.year}</p>
                    </div>
                </div>
            ...
            <div className={styles["card-bottom"]}>
                <p className={styles["genre"]}> {process_genre(book.genre)} </p>
